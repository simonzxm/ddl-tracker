...
export class PostgresCatalogImportRepository implements CatalogImportRepository {
  ...
  async loadPlanningContext(
    ...
    const baseline = await this.#loadBaseline(importRecord.term.external_code);
  ...
  async #loadBaseline(externalTermCode: string): Promise<CatalogBaseline> {
    const termResult = await this.#client.query<TermRow>(
    ...
    const courseResult = await this.#client.query<CourseRow>(
    ...
    const sectionResult = await this.#client.query<SectionRow>(
