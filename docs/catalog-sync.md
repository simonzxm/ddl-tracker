...
生产 Worker 的 Cron Trigger 每天 `03:17 UTC` 执行一次同步。同步 module 读取上游 `main` 分支当前 commit，锁定该 commit 后发现并下载：

```text
data/<YYYY-YYYY-[1|2|3]>/courses.csv.gz
```

所有文件都从固定 commit 的 raw URL 下载，因此同一次运行不会混用不同上游版本。上游 tree 响应被截断、commit/blob metadata 非法、下载失败、gzip 损坏或文件超过限制时，本次对应学期同步失败。

## 同步范围与进度

- `catalog_sync_state` 为每个 `(repository, term_code)` 保存最近成功同步的 commit SHA、blob SHA、CSV checksum 与 run ID。
- blob SHA 未变化的学期直接跳过，不重新下载。
- 每次 Cron 最多处理 4 个发生变化的学期，按学期代码从新到旧排序。
- 首次部署会优先同步最近学期；剩余历史学期由后续 Cron 继续补齐。
- 每个学期独立提交。一个学期失败时停止本次运行，已经成功提交的其他学期不回滚；失败学期不会更新 `catalog_sync_state`，下次 Cron 会重试。

## 下载与资源限制

- GitHub API 请求获取 `main` commit 和 recursive tree。
- 只接受路径严格匹配 `data/<term>/courses.csv.gz` 的 blob。
- gzip 最大 4 MiB。
- 解压后 CSV 最大 10 MiB。
...
| `source_metadata` | repository、commit SHA、blob SHA、source path、run ID、checksum |
...
上游 blob 变化但规范化目录内容未变化时，仍更新同步 state 和成功 run，但不推进目录 revision，也不产生目录变更事件。

## 运行记录与故障处理

`catalog_sync_runs` 长期记录：repository、commit SHA、term code、source path、blob SHA、CSV checksum、行数、课程/教学班计数、是否产生目录变化、diff、状态、错误摘要和起止时间。
...
2. 检查 GitHub API/raw 可达性与上游 tree 是否被截断。
3. 检查最近失败的 `catalog_sync_runs`，确认 term、commit、blob 与错误摘要。
4. 修复代码、资源限制或等待上游数据修正；下一次 Cron 会自动重试未成功 blob。
